...
import 'package:fmp/screens/standings2017.dart';
import 'package:shared_preferences/shared_preferences.dart';

import '../nav_bottom.dart';

class SoccerTwoThousandSeventeen extends StatefulWidget {
  const SoccerTwoThousandSeventeen({Key? key}) : super(key: key);

  @override
  _SoccerTwoThousandSeventeenState createState() =>
      _SoccerTwoThousandSeventeenState();
}

class _SoccerTwoThousandSeventeenState
    extends State<SoccerTwoThousandSeventeen> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          IconButton(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const Standings2017(),
                ),
              );
            },
      ...
      body: FutureBuilder(
        future: SoccerApi()
            .getAllMatches2017(), //Here we will call our getData() method,
        builder: (context, snapshot) {
          //the future builder is very intersting to use when you work with api
          if (snapshot.hasData) {
            List<SoccerMatch> allmatches = snapshot.data as List<SoccerMatch>;
            print(allmatches);
            return PageBody(allmatches: allmatches, year: "2017");
          } else {
            return const Center(
