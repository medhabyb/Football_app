...
import 'package:fmp/nav_bottom.dart';
...
import 'package:fmp/screens/standings2018.dart';
import 'package:shared_preferences/shared_preferences.dart';

class SoccerTwoThousandEighteen extends StatefulWidget {
  const SoccerTwoThousandEighteen({Key? key}) : super(key: key);

  @override
  _SoccerTwoThousandEighteenState createState() =>
      _SoccerTwoThousandEighteenState();
}

class _SoccerTwoThousandEighteenState extends State<SoccerTwoThousandEighteen> {
  @override
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          IconButton(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const Standings2018(),
                ),
              );
            },
      ...
      body: FutureBuilder(
        future: SoccerApi()
            .getAllMatches2018(), //Here we will call our getData() method,
        builder: (context, snapshot) {
          //the future builder is very intersting to use when you work with api
          if (snapshot.hasData) {
            List<SoccerMatch> allmatches = snapshot.data as List<SoccerMatch>;
            print(allmatches);
            return PageBody(allmatches: allmatches, year: "2018");
          } else {
            return const Center(
